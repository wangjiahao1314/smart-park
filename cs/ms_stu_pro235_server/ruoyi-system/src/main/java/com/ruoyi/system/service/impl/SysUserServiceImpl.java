...
public class SysUserServiceImpl implements ISysUserService {
    ...
    @Autowired
    private IParkTenantService tenantService;

    @Autowired
    private IParkWorkerService workerService;
    ...
    @Override
    @Transactional
    public boolean registerUser(SysUser user, String roleKey, String idCardNumber) {
        // 根据roleKey获取roleId
        Long roleId = roleMapper.selectRoleIdByRoleKey(roleKey);
        if (roleId == null) {
            throw new ServiceException("租户角色权限字符信息有误");
        }
        ...
        boolean b = false;
        // 添加tenant表信息
        if (Constants.TENANT.equals(roleKey)) {
            ParkTenant tenant = new ParkTenant();
            tenant.setIdCardNumber(idCardNumber);
            tenant.setUserId(user.getUserId());
            b = tenantService.save(tenant);
        } else if (Constants.REPAIR.equals(roleKey)) {
            ParkWorker worker = new ParkWorker();
            worker.setIdCardNumber(idCardNumber);
            worker.setUserId(user.getUserId());
            b = workerService.save(worker);
        }

        if (!b) {
            throw new ServiceException("插入： " + roleKey + "信息有误");
        }
